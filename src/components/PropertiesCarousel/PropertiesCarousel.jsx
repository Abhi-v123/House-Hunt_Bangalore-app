...
},
{
  ...
  img: "https://99realty.in/wp-content/uploads/2024/09/Urban-Heights-1.jpg",
  forRent: true
},
{
  id: 28,
...
export default function PropertiesCarousel() {
  ...
  const handleBook = () => {
    ...
    if (loggedInUser) {
      setMessage({ type: "success", text: "✅ Thanks for showing interest! Our agent will contact you soon." });
    } else {
      setMessage({ type: "error", text: "❌ Please login to book this property." });
    }
  };
          ...
          {properties.map((p) => (
            <div className="property-card" key={p.id}>
              ...
              <div className="card-info">
                ...
                <p className="card-location"><FaMapMarkerAlt /> {p.location}</p>
                <div className="details-list">
                  {p.beds !== "-" && <span><FaBed /> {p.beds} Beds</span>}
                  {p.baths !== "-" && <span><FaBath /> {p.baths} Baths</span>}
                  <span><FaRulerCombined /> {p.area}</span>
                </div>
                <button className="details-button" onClick={() => { setSelectedProperty(p); setMessage(null); }}>See Details</button>
              </div>
            </div>
          ))}
        </div>
  ...
  <div className="modal-overlay" onClick={() => setSelectedProperty(null)}>
    <div className="modal-content" onClick={(e) => e.stopPropagation()}>
      <img src={selectedProperty.img} alt={selectedProperty.title} className="modal-img" />
      <h2 className="modal-title">{selectedProperty.title}</h2>
      <div className="modal-details">
        <p><span className="modal-label">Price:</span> <span className="modal-strong">{selectedProperty.price}</span></p>
        <p><span className="modal-label">Location:</span> <span className="modal-green">{selectedProperty.location}</span></p>
        <p><span className="modal-label">Area:</span> <span className="modal-green">{selectedProperty.area}</span></p>
      ...
      <div className="button-group">
        <button className="book-button" onClick={handleBook}>Book Property</button>
